...
const firstLetters = (strs) => {
  return strs.map((idx) => idx.charAt(0));
};
...
const findHappiness = (strs) => {
  return strs.filter((idx) => idx.includes(':)'));
};
...
const unenrollBrook = (roster) => {
  return roster.map((classArr) => classArr.filter((student) => !student.includes('Brook')));
};
...
const characterByIndex = (strs) => {
  return strs.map((str, i) => str.charAt(i));
};
