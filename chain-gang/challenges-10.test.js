...
const count = (target, input) => {
  return input.reduce((x, y) => x.concat(y), []).filter((num) => num === target).reduce((count) => count += 1, 0);
};
...
const totalSum = (input) => {
  return input.map((arr) => arr.reduce((sum, int) => sum += int, 0)).reduce((sum, int) => sum += int, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((arr) => arr.filter((num) => typeof(num) === 'number' && num % 5 === 0).map((result) => Math.pow(2, result)));
};
...
let findShortest = (data) => {
  return data.reduce((shortest, character) => parseInt(character.height) < parseInt(shortest.height) ? character : shortest, { height: Number.MAX_SAFE_INTEGER }).name;
};
